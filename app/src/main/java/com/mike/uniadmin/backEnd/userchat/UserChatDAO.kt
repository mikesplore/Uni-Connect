...
interface UserChatDAO {
    ...
    SELECT uc.*, 
           ...
           senderState.online AS sender_state,
           ...
           receiverState.online AS receiver_state,
           unreadCounts.unreadCount
    FROM userChats uc
    INNER JOIN admins AS sender ON uc.senderID = sender.id
    LEFT JOIN admins AS receiver ON uc.recipientID = receiver.id
    LEFT JOIN userState AS senderState ON sender.id = senderState.userID
    LEFT JOIN userState AS receiverState ON receiver.id = receiverState.userID
    INNER JOIN (
        SELECT senderID, MAX(timeStamp) AS latestTimestamp
        FROM userChats
        GROUP BY senderID
    ) latestChats ON uc.senderID = latestChats.senderID AND uc.timeStamp = latestChats.latestTimestamp
    LEFT JOIN (
        SELECT senderID, COUNT(*) AS unreadCount
        FROM userChats
        WHERE deliveryStatus != 'READ' AND senderID != :currentUserId 
        GROUP BY senderID
