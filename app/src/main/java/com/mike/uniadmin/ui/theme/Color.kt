package com.mike.uniadmin.ui.theme

import androidx.compose.ui.graphics.Color


/*⚠️
    The color names use here are not the actual names
    I was changing the codes to find the best palette for the app
  ⚠️
*/


//light theme
val LightBlue = Color(0xFFDCEEFB)
val PaleBlue = Color(0xFF7D8ABC)
val Cyan = Color(0xFF2980B9)
val DarkGray = Color(0xFF212121)
val SkyBlue = Color(0xFFBBDEFB)
val White = Color(0xFF1B4F72)


//dark theme
val DarkNavy = Color(0xFF15202B)
val DeepBlue = Color(0xFF1B4F72)
val BrightBlue = Color(0xFF7D8ABC)
val LightGray = Color(0xFFE1E8ED)
val Turquoise = Color(0xFF1C2833)
val Black = Color(0xFF2980B9)



